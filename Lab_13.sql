...
select v.Name,c.Name from City c join Village v on c.CityID=v.CityID where c.Name='Rajkot'

select c.Name as City_Name,v.Name as Village_Name,c.Pincode from City c join Village v on c.CityID=v.CityID 

select c.Name as City_Name,count(v.CityId) from City c join Village v on c.CityID=v.CityID group by c.Name having count(v.CityId)>1

select c.Name as City_Name,count(v.CityId) from City c full outer join Village v on c.CityID=v.CityID group by c.Name having count(v.CityId)=0

select c.Name as City_Name,count(v.CityId) from City c full outer join Village v on c.CityID=v.CityID group by c.Name

select count(c.CityID) from City c left outer join Village v on c.CityID=v.CityID group by c.Name having count(v.CityId)>1	 
...
Spi decimal(8,2) check (Spi<=10 and Spi>=0),
Bklog int Check (Bklog>=0)
);
 
select * from STU_MASTER

Insert into STU_MASTER values(101,'Raju','CE',8.80,0),(102,'Amit','CE',2.20,3),(103,'Sanjay','ME',1.50,6),(104,'Neha','EC',7.65,0),(105,'Meera','EE',5.52,2),(106,'Mahesh',default,4.50,3)
